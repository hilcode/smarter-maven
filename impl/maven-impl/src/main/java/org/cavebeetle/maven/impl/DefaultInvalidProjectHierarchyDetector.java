 ...
 */
@Singleton
public final class DefaultInvalidProjectHierarchyDetector implements InvalidProjectHierarchyDetector {
            ...
            if (!snapshotDetector.isSnapshot(mavenProject)) {
                ...
                if (maybeParentMavenProject.isPresent()) {
                    ...
                    if (snapshotDetector.isSnapshot(parentMavenProject)) {
                        ...
                        final String groupAndArtifact =
                                parentMavenProject.getGroupId() + ":" + parentMavenProject.getArtifactId();
                        maxGroupAndArtifactLength =
                                checkMaxGroupAndArtifactLength(maxGroupAndArtifactLength, groupAndArtifact);
                    }
                }
                ...
                if (!snapshotGavs.isEmpty()) {
                    ...
                    for (final Gav dependencyGav : snapshotGavs) {
                        final String groupAndArtifact =
                                dependencyGav.getGroupId() + ":" + dependencyGav.getArtifactId();
    ...
    public List<String> getProjectHierarchyWarnings(final GavToProjectMap gavToProjectMap) {
        ...
        for (final Gav gav : map.keySet()) {
            ...
            if (!gavTuples.isEmpty()) {
                ...
                warnings.add("");
                warnings.add(
                        "Project " + gav.getGroupId() + ":" + gav.getArtifactId() + " has version " + gav.getVersion());
                warnings.add("");
                warnings.add("    but");
                warnings.add("");
                int maxGroupAndArtifactLength = -1;
    ...
    private Optional<Gav> findClosestGav(final Gav gav_, final GavToProjectMap gavToProjectMap) {
        for (final Gav gav : gavToProjectMap) {
            if (gav.getGroupId().equals(gav_.getGroupId())
                    && gav.getArtifactId().equals(gav_.getArtifactId())) {
                return of(gav);
            }
        }
        return absent();
