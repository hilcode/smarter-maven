 ...
 */
@Singleton
public final class DefaultDirtDetector implements DirtDetector {
    /**
    ...
    public DirtyReason findDirtyReason(
        ...
        if (!snapshotDetector.isSnapshot(mavenProject)
                && artifactDetector.hasArtifactInRemoteRepository(mavenProject)) {
            return PUBLISHED;
        }
        ...
        final int digestLength =
                expectedProjectSourceFilesDigest.getDigest().toString().length();
        ...
        if (!isEqual(expectedProjectSourceFilesDigest, actualProjectSourceFilesDigest)) {
            return newFileChangedDirtyReason(
                    digestLength, files, expectedFileToDigestMap, actualProjectSourceFilesDigest);
        }
        ...
        if (maybeFirstDirtyDependency.isPresent()) {
            return internalApi.newDirtyReason(
                    true, maybeFirstDirtyDependency.get().getGav().toString());
        }
        return NOT_DIRTY;
    }
