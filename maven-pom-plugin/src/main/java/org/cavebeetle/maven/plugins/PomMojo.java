...
public final class PomMojo {
    public static final void main(final String[] argv) throws Exception {
        ...
        final XMLStreamReader2 xmlStreamReader =
                (XMLStreamReader2) factory.createXMLStreamReader(new StringReader(pomContent));
        Tree.Branch tree = new Tree.Branch();
        final LocationInfo firstLocation = xmlStreamReader.getLocationInfo();
        final Tree.Leaf leaf = new Tree.Leaf(
                Tree.LeafType.TEXT, pomContent.substring((int) firstLocation.getStartingCharOffset(), (int)
                        firstLocation.getEndingCharOffset()));
        tree.addTree(leaf);
        ...
        while (xmlStreamReader.hasNext()) {
            ...
            final String text = pomContent.substring(
                    (int) currentLocation.getStartingCharOffset(), (int) currentLocation.getEndingCharOffset());
            switch (eventType) {
                case START_ELEMENT: {
                ...
                case CHARACTERS:
                case SPACE: {
                    tree.addTree(new Tree.Leaf(
                            Tree.LeafType.TEXT,
                            pomContent.substring((int) currentLocation.getStartingCharOffset(), (int)
                                    currentLocation.getEndingCharOffset())));
                    break;
                }
                case END_DOCUMENT: {
                    tree.addTree(new Tree.Leaf(
    ...
    public static final Tree merge(final Tree.Branch tree) {
        ...
        for (final Tree leafOrBranch : tree) {
            if (leafOrBranch.isBranch()) {
                ...
                if (previousLeaf != null) {
                    if (previousLeaf.isText()
                            && leafOrBranch.asLeaf().getLeafType() == Tree.LeafType.START_ARTIFACT_ID) {
                        mergedTree.addTree(new Tree.Leaf(
                                Tree.LeafType.START_ARTIFACT_ID, previousLeaf.toString() + leafOrBranch.toString()));
                    } else {
                        mergedTree.addTree(previousLeaf);
                    }
                    previousLeaf = null;
